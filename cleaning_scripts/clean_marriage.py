#!/usr/bin/env python3
#cleaning script for cpni07 - marital status over time
...
TABLE_PREFIX = "CPNI07"
OUT_PATH = CLEAN_DIR / "marriage.csv"
...
def clean_marriage(raw_path: Path) -> pd.DataFrame:
    ...
    df["Marital Status"] = clean_string_column(df["Marital Status"])
    ...
    #pivot UNIT -> columns
    out = (
        df[["Year", "Sex", "Status", "Region", "UNIT_M", "VALUE"]]
        .rename(columns={"UNIT_M": "unit", "VALUE": "value"})
        .pivot_table(
            index=["Year", "Sex", "Status", "Region"],
            columns="unit",
            values="value",
            aggfunc="first",
        )
    ...
    out = out.sort_values(["Year", "Sex", "Status", "Region"]).reset_index(drop=True)
...
def main() -> None:
    ...
    cleaned = clean_marriage(raw_path)
